...
@Configuration // context 파일로 인식됨.
public class ProjectConfig4 {

    //java context 파일로 주입하는 방법 2가지
    // 1. set()메서드 이용 : 잘 사용하지 않음. 먼저 Person이 만들어진 후에 parrot을 만들어 나중에 집어넣는 방식
    ...
    public Person person(Parrot parrot){
        Person p = new Person(); // 객체 생성 후, 주입하는 경우 set 메서드 이용해서 주입 가능!
        // Person p = new Person(parrot); // 생성자 주입 방법
        p.setName("Ella");
        // p.setParrot(parrot()); // 주입할 때 set메서드 호출할 수 있음
        p.setParrot(parrot);
        //Person만들기 전에 이미 만들어진 parrot 싱글톤 객체를 찾아서 Person 객체 만들 때, 주입
        return p;
    }
}
